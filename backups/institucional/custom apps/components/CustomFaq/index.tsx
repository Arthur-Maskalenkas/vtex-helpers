...
import { useCustomFaq } from "../../context/CustomFaqContext"
...
const CustomFaqQuestions = () => {
  ...
  const css = useCssHandles(handles)
  ...
  return (
    <div className={css['customFaq-container']}>
      {questions?.map(({ __editorItemTitle, response, colorQ, colorR, colorQIcon, isTypeTitle }: CustomFaqQuestionsProps, index) => {
        if (isTypeTitle) {
          return (
            <h2 className={css['customFaq-only-title']}>
              {__editorItemTitle}
            </h2>
          )
        }
