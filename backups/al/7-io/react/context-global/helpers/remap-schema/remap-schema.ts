...
export class RemapSchema {
  ...
  public remap(): schema_site_editor_remapped_root_custom_app_badge_custom | null {
    ...
    return {
      ...(Object?.keys(this.schema) as any)?.reduce((acc: schema_site_editor_remapped_root_custom_app_badge_custom, key: keyof schema_site_editor_default_root_app_badge_custom) => {
        ...
        console.log('currentQuadrantContainer', currentQuadrantContainer)
            ...
            listOfFlags: currentQuadrantContainer?.collection
              ...
              ?.map((collection) => ({
                ...
                badgesStyles: collection?.typeContent === 'image'
                  ? ({
                    src: (collection?.items as any)?.[0]?.src,
                    alt: (collection?.items as any)?.[0]?.alt,
                    width: (collection?.items)?.[0]?.commonPropsBetweenContentAndImage?.width ?? 'auto',
                    height: (collection?.items)?.[0]?.commonPropsBetweenContentAndImage?.height ?? 'auto'
                  })
                  : ({
                    text: (collection?.items as any)?.[0]?.text,
                    fontSize: (collection?.items as any)?.[0]?.fontSize || '14px',
                    lineHeight: (collection?.items as any)?.[0]?.fontSize || '14px',
                    color: (collection?.items as any)?.[0]?.color || '#000',
                    fontWeight: (collection?.items as any)?.[0]?.fontWeight || '400',
                    borderRadius: (collection?.items as any)?.[0]?.borderRadius || '0px',
                    backgroundColor: (collection?.items as any)?.[0]?.backgroundColor || '#fff',
                    width: (collection?.items)?.[0]?.commonPropsBetweenContentAndImage?.width || 'auto',
                    height: (collection?.items)?.[0]?.commonPropsBetweenContentAndImage?.height || 'auto',
                    display: 'flex',
                    alignItems: 'center',
