...
type CouponInBannerDesktopProps = {
	banner: string
	couponCode: string,
	showBanner: boolean,
...
export const CouponInBannerDesktop = (
	{
		banner = '',
		showBanner = false,
		couponCode = '',
			...
			<ToastProvider positioning="window">
				<ToastConsumer>
					{({showToast}: { showToast: any }) => (
						<div className={`${styles.containerCouponInBannerDesktop} ${styles[showBanner ?
							'withBanner' :
							'withoutBanner']} ${classnameWithStatus()}`}>
							{showBanner && (
								<button aria-label={"Adicionar cupom ao carrinho"}>
									<img onClick={() => handleClickBanner(showToast)}
											 className={styles.containerImageDesktop} src={banner}
											 alt={"Banner De Cupom"}/>
								</button>
							)}
...
CouponInBannerDesktop.schema = {
	title: "Cupom Banner Desktop"
}
