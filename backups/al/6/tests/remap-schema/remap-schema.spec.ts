...
import {
  mock_remap_real_data_refined_1,
  mock_remap_real_data_result_1,
  mock_remap_real_data_schema_1
} from './mocks/integration-mocks/_data-1'
...
const makeSut = (schema: schema_site_editor_default_root_app_badge_custom | null, refinedDataOnroductContext: DataRefined = mock_refined_data_on_product_context): SutTypes => {
  const FilterMock = new FilterDefaultSchema(refinedDataOnroductContext)
  const Sorter = new SortMatchs()
  const sut = new RemapSchema(schema, FilterMock,Sorter)
...
describe('Remap Schema', () => {
  describe('Filter Integration', () => {
    it('Should call Filter with valid collection on quadrant bottom', () => {
      ...
      sut.remap()
      expect(spyFilter).toHaveBeenCalledWith(mock_schema_default.quadrantBottom?.[0].collection[0])
    })
    ...
    it('Should call Filter with valid collection on quadrant top left', () => {
      ...
      const spyFilter = jest.spyOn(FilterMock, 'filterCollection')
      sut.remap()
      expect(spyFilter).toHaveBeenCalledWith(mock_schema_default.quadrantTopLeft?.[0].collection[0])
    })
    ...
    it('Should call Filter with valid collection on quadrant top right', () => {
      ...
      const spyFilter = jest.spyOn(FilterMock, 'filterCollection')
      sut.remap()
      expect(spyFilter).toHaveBeenCalledWith(mock_schema_default.quadrantTopRight?.[0].collection[0])
    })
    ...
    it('Should return html data from collecions matched', () => {
      const mock_variation_value: FieldDataRefined = {
      ...
      const mock_variation_result_html = {
        [`data-custom--match-by-${propertieToHTMLMatchBy}--name-${normalizeString(mock_variation_value.name)}--value-${normalizeString(mock_variation_value.value[0])}`]: true
      }
          ...
          linksByVariation: [
            {
              ...
              value: mock_variation_value.value[0]
            }
          ]
        }]
      ...
      const mock_refined_data = generateRefinedData({
        variations: [
          {
            name: mock_variation_value.name,
            values: mock_variation_value.value
          }
        ]
      })
  ...
  describe('ingration with real data', () => {
    it('Should Work Well With Only Quadrant Right', () => {
      const mock_schema = mock_remap_real_data_schema_1
      const mock_refined = mock_remap_real_data_refined_1
      const mock_result = mock_remap_real_data_result_1

      const { sut } = makeSut(mock_schema, mock_refined)
