import type { EventName, PixelData } from 'vtex.pixel-manager/react/PixelEventTypes';
...
function getNameWithoutVariant(item: CartItem) {
  if ((item?.name && !item.name.includes(item.skuName)) || item.name === item.skuName) {
    return item.name;
  }
...
export function removesUnpexpectedClickGtm() {
  ...
  const dataLayerLastButOneEvent: DataLayerObject = dataLayer[dataLayer.length - 2];
...
export const updateDatalayer = (action: EventName, productContextValue: any, quantityDispatch: number) => {
  ...
  if (productItem.sellers[0].commertialOffer.ListPrice) {
    productCalculatedPrice = Number((productItem.sellers[0].commertialOffer.ListPrice * productItem.unitMultiplier).toFixed(2));
  }
  if (productItem.sellers[0].commertialOffer.Price) {
    productCalculatedPrice = Number((productItem.sellers[0].commertialOffer.Price * productItem.unitMultiplier).toFixed(2));
  }
  ...
  if (action === 'addToCart') {
    updateDatalayerObject({ event: action, ecommerce: { add: { products: [dataLayerObject] } } });
  } else if (action === 'removeFromCart') {
    updateDatalayerObject({ event: action, ecommerce: { remove: { products: [dataLayerObject] } } });
  }
};
