...
function getBestPrice(item: ProductTypes.Item, condition: PriceConditionRule) {
	...
	if (condition === 'highest') {
		itemPrice = allPrices.reduce((max: any, price: any) => (price > max
																														? price
																														: max))
	} else {
		itemPrice = allPrices.reduce((min: any, price: any) => (price < min
																														? price
																														: min))
	}
	...
	if (condition === 'highest') {
		itemToReturn = bestPrices.reduce(
			(maxIndex, currentPrice, currentIndex) =>
				currentPrice > bestPrices[maxIndex]
				? currentIndex
				: maxIndex,
			0
		)
		...
		itemToReturn = bestPrices.reduce(
			(minIndex, currentPrice, currentIndex) =>
				currentPrice < bestPrices[minIndex]
				? currentIndex
				: minIndex,
			0
		)
