...
export const buildImageAttrs = ({ widthImg = '100', heightImg = '100', className, ...imgProps }: BuildImageAttrsProps): Partial<React.ImgHTMLAttributes<HTMLImageElement>> => {
  const data = {
    ...imgProps,
    loading: 'lazy',
    decoding: 'async',
    height: Number(heightImg),
    width: Number(widthImg),
    className
...
function getBestPrice(item: ProductTypes.Item, condition: PriceConditionRule) {
  ...
  if (condition === 'highest') {
    itemPrice = allPrices.reduce((max: any, price: any) => (price > max ? price : max))
  } else {
    itemPrice = allPrices.reduce((min: any, price: any) => (price < min ? price : min))
  }
